...
const websiteOrders = ss.getSheetByName("Website Orders");
...
const ygntransfers = SpreadsheetApp.openById(
  "1Glo17mHrnGPelyeKGkmvVBVWpmRsn8v-zISXX2d_EHo"
).getSheetByName("Transfers");
...
const pmntoolsrevenue = SpreadsheetApp.openById(
...
************************************/
let onEditTriggersPMNToolsShop = (e) => {
...
let saveCart = (signature, cart, e) => {
  let websiteOrderNum = cart.getRange(4, 8).getValue();
  let websiteOrderStartRow = cart.getRange(6, 12).getValue();
  let nextInvoiceRow = invoice.getLastRow() + 1;
  let invoiceNumber = cart.getRange(10, 10).getValue();
  ...
  let itemColumn = cart.getRange(10, 9).getValue() - 1;
  ...
  for (const data of cartArrayTwo) {
    ...
    let cosignmentArray = invCosignments
      ...
      .getValues();
    let PMN = items.getRange(itemRow, 7).getValue();
    let initialBalance;
    let currentBalance;
    console.log(code);
    if (location == "PMN") {
      initialBalance = PMN;
      currentBalance = PMN - qty;
    }
    addItemsArray[l] = [];
  ...
  if (websiteOrderNum) {
    fulfillWebsiteOrder(websiteOrderNum, signature, websiteOrderStartRow);
  }
  let invoiceCheck =
    cart.getName() == "Cart"
...
/***********************************
- Fullfill Website Order
************************************/
let fulfillWebsiteOrder = (websiteOrderNum, signature, startRow) => {
  let orders = websiteOrders
    .getRange(2, 2, websiteOrders.getLastRow(), 1)
    .getValues();
  let fulfillArray = [];
  for (i = 0; i < orders.length; i++) {
    if (orders[i][0] == websiteOrderNum) {
      fulfillArray.push([signature]);
    }
  }
  websiteOrders
    .getRange(startRow, 14, fulfillArray.length, 1)
    .setValues(fulfillArray);
};
/***********************************
- Separate Cosignment
************************************/
...
let returnItem = (invoice, itemAmount, e) => {
  ...
  let dateReturn = returnCancel.getRange(11, 2).getValue();
  let PMN = items.getRange(itemRow, 7).getValue();
...
let cancelInvoice = (e) => {
  ...
  invoice.getRange(startInvoiceRow, 20, j, 1).setValue("Canceled");
  e.range.setValue(false);
  movementData
    .getRange(movementData.getLastRow() + 1, 1, stockMovementArray.length, 9)
    .setValues(stockMovementArray);
...
let payCredit = (amount) => {
  ...
  if (amount > amtLeft) {
    ...
    for (let j = 0; j < 21; j++) {
      ...
      invoice.getRange(creditInvoiceRowInReceipt + j, 15).setValue(salePrice);
      invoice.getRange(creditInvoiceRowInReceipt + j, 16).setValue(date);
    }
...
let dailyExpenses = (signature) => {
  ...
  let expenses = cashBook.getRange(8, 13, 28, 2).getValues();
  ...
  for (let i = 0; i < expenseDate.length; i++) {
    ...
    if (date == checkDate && checkExpense == "PMN-TOOLS") {
  ...
  for (let i = 0; i < expenses.length; i++) {
    ...
    let expense = expenses[i][0];
    let amount = expenses[i][1];
  ...
  expenseSheet
    .getRange(expenseLastRow, 1, expenseArray.length, 5)
    .setValues(expenseArray);
  cashBook.getRange(8, 13, 28, 2).setValue("");
  let revenue = cashBook.getRange(14, 11).getValue();
  dailyRevenue(
    signature,
    cashBook.getRange(4, 7).getValue(),
    revenue,
...
let dailyRevenue = (signature, date, revenue, revenueLastRow) => {
  let dayRevenue = cashBook.getRange(3, 11).getValue();
  let dayIncome = cashBook.getRange(7, 11).getValue();
  let system =
    revenueSheet.getRange(revenueSheet.getLastRow(), 7).getValue() + 1;
  let revenueArray = [
    [date, "PMN-TOOLS", revenue, dayRevenue, dayIncome, signature, system],
  ];
  ...
  let array = [
    ...
    [, , , 96006, "Open Trade Debtor-PMN Power Tools", dayRevenue, ,],
    [, , , 70028, "Revenue-Open Power Tools (PMN)", , dayRevenue],
    [transactionNum + 2, date, systemRevenue, "Daily Income", , , ,],
    [, , , 10004, "Cash in Hand-PMN Tools", dayIncome, ,],
    [, , , 96006, "Open Trade Debtor-PMN Power Tools", , dayIncome],
  ];
  ...
  transactionsSheet
    ...
    .setValues(array);
  pmntoolsrevenue
    .getRange(pmntoolsrevenue.getLastRow() + 1, 1, 6, 7)
    .setValues(array);
};
...
let createTransfer = (signature) => {
  ...
  newInvoiceNum = "pmntoolstransfer-" + newInvoiceNum;
...
let confirmTransfer = (invoiceNum, e) => {
  ...
  for (data of invoiceArray) {
    ...
    let tgn = ygnItems.getRange(itemRow, 8).getValue();
    let dagon = ygnItems.getRange(itemRow, 9).getValue();
    let jsix = ygnItems.getRange(itemRow, 10).getValue();
    let sr = ygnItems.getRange(itemRow, 7).getValue();
    let PMN = items.getRange(itemRow, 7).getValue();
    let lastRowTo;

    if (to == "Showroom" || to == "TGN" || to == "DAGON" || to == "JSIX") {
      lastRowTo = ygnTransferLastRow;
    } else {
    ...
    if (from == "PMN") {
      ...
      if (from == "PMN") {
        from = 7;
        initialBalanceOne = PMN;
        currentBalanceOne = PMN - qty;
      }
    ...
    if (to == "PMN") {
      ...
      if (to == "PMN") {
        to = 7;
        initialBalanceTwo = PMN;
        currentBalanceTwo = PMN + qty;
      }
...
let createLocalPurchase = (signature) => {
  ...
  let newInvoiceNum = Number(invoiceNum.split("-")[1]) + 1;
  newInvoiceNum = "pmntoolsLocalPurchase-" + newInvoiceNum;
  ...
  for (data of invoiceArray) {
    ...
    cosignmentArray[j] = [
      ...
      "32007",
      "PMN",
      "=SUMIF('YGN Receipt'!A:A,A" +
        Number(lastCoRow) +
        ",'YGN Receipt'!H:H)+SUMIF('MDY Receipt'!A:A,A" +
        Number(lastCoRow) +
        ",'MDY Receipt'!H:H)+SUMIF('PMN Receipt'!A:A,A" +
        Number(lastCoRow) +
        ",'PMN Receipt'!H:H)",
