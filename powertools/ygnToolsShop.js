...
const ygntoolsRevenue = SpreadsheetApp.openById(
...
************************************/
let onEditTriggersYGNToolsShop = (e) => {
...
let saveCart = (signature, cart, e) => {
  ...
  let itemColumn = cart.getRange(10, 9).getValue() - 5;
  ...
  for (const data of cartArrayTwo) {
    ...
    if (data[3] > invCosignments.getRange(data[10], 21).getValue()) {
  ...
  for (const data of cartArrayTwo) {
    ...
    let cosignmentArray = invCosignments
      ...
      .getValues();
    let sr = items.getRange(itemRow, 7).getValue();
    let tgn = items.getRange(itemRow, 8).getValue();
    let dagon = items.getRange(itemRow, 9).getValue();
    let jsix = items.getRange(itemRow, 10).getValue();
    let initialBalance;
    let currentBalance;
    if (location == "Showroom") {
      initialBalance = sr;
      currentBalance = sr - qty;
    }
    if (location == "TGN") {
      initialBalance = tgn;
    ...
    let itemCode = accountingItems.getRange("A1:A").getValues();
    for (let i = 0; i < itemCode.length; i++) {
      if (code == itemCode[i]) {
        accountingItems.getRange(itemRow, 8, 1, 1).setValue("True");
        accountingItems.getRange(itemRow, 8, 1, 1).setBackground("#003B46");
      }
    }
  }

  invoice
    .getRange(nextInvoiceRow, 1, cartArray.length, 19)
    .setValues(cartArray);
  movementData
    .getRange(movementData.getLastRow() + 1, 1, stockMovementArray.length, 10)
...
let returnItem = (invoice, itemAmount, e) => {
  ...
  let nextInvoiceRow = invoice.getLastRow() + 1;
  let leftamount = returnCancel.getRange(4, 13).getValue();
  let payamount = returnCancel.getRange(5, 13).getValue();
  ...
  invoice
    .getRange(nextInvoiceRow, 1, returnArray.length, 19)
  ...
  creditPaymentArray = [
    [invoiceNumber, date, customer, "", "", allAmount, "Return"],
  ];
...
let cancelInvoice = (e) => {
  ...
  let l = 0;
  let alreadyCancel =
    invoice.getRange(startInvoiceRow, 20).getValue() == "Canceled"
      ? true
      : false;
...
let dailyExpenses = (signature) => {
  ...
  let expenses = cashBook.getRange(8, 12, 29, 4).getValues();
  ...
  for (let i = 0; i < expenseDate.length; i++) {
    ...
    if (date == checkDate && checkExpense == "YGN-TOOLS") {
  ...
  cashBook.getRange(8, 12, 29, 4).setValue("");
...
let dailyRevenue = (signature, date, revenue, revenueLastRow) => {
  ...
  let revenueArray = [
    [date, "YGN-TOOLS", revenue, dayRevenue, dayIncome, signature, system],
  ];
  ...
  let array = [
    ...
    [, , , 96004, "Open Trade Debtor-YGN Power Tools", dayRevenue, ,],
    [, , , 70025, "Revenue-Open Power Tools (YGN)", , dayRevenue],
    [transactionNum + 2, date, systemRevenue, "Daily Income", , , ,],
    [, , , 10016, "Cash in Hand-YGN Tools", dayIncome, ,],
    [, , , 96004, "Open Trade Debtor-YGN Power Tools", , dayIncome],
  ];
  ...
  transactionsSheet
    ...
    .setValues(array);
  ygntoolsRevenue
    .getRange(ygntoolsRevenue.getLastRow() + 1, 1, 6, 6)
...
let createTransfer = (signature) => {
  ...
  newInvoiceNum = "ygntoolstransfer-" + newInvoiceNum;
...
let confirmTransfer = (invoiceNum, e) => {
  ...
  let pmnTransferLastRow = pmntransfers.getLastRow() + 1;
  let stockMovementArray = [];
  ...
  for (data of invoiceArray) {
    ...
    let remark = data[8];
    let thirtyTwoStock = mdyItems.getRange(itemRow, 7).getValue();
    let eightyFourStock = mdyItems.getRange(itemRow, 8).getValue();
    let tgn = items.getRange(itemRow, 8).getValue();
    let dagon = items.getRange(itemRow, 9).getValue();
    let jsix = items.getRange(itemRow, 10).getValue();
    let sr = items.getRange(itemRow, 7).getValue();
    let PMN = pmnItems.getRange(itemRow, 7).getValue();
    let lastRowTo;
    if (to == "32ND" || to == "84TH") {
      lastRowTo = mdyTransferLastRow;
    } else {
      lastRowTo = pmnTransferLastRow;
    }
    ...
    itemArrayTwo[l] = [
      ...
      remark,
      "No",
      "=MATCH(F" + Number(lastRowTo + l) + ",Items!A:A,0)",
    ];
    ...
    if (
      ...
      if (from == "Showroom") {
        from = 7;
        initialBalanceOne = sr;
        currentBalanceOne = sr - qty;
      }
      if (from == "TGN") {
        from = 8;
    ...
    if (to == "Showroom" || to == "TGN" || to == "DAGON" || to == "JSIX") {
      let initialBalanceTwo;
      let currentBalanceTwo;

      if (to == "Showroom") {
        to = 7;
        initialBalanceTwo = sr;
        currentBalanceTwo = sr + qty;
      }
      if (to == "TGN") {
        to = 8;
  ...
  if (to == "PMN") {
    pmntransfers
      .getRange(pmnTransferLastRow, 1, itemArrayTwo.length, 12)
      .setValues(itemArrayTwo);
    pmntransfers
      .getRange(pmnTransferLastRow, 1, itemArrayTwo.length, 1)
      .insertCheckboxes();
  }
...
let createLocalPurchase = (signature) => {
  ...
  let newInvoiceNum = Number(invoiceNum.split("-")[1]) + 1;
  newInvoiceNum = "ygntoolsLocalPurchase-" + newInvoiceNum;
  ...
  for (data of invoiceArray) {
    ...
    cosignmentArray[j] = [
      ...
      date,
      qty,
      "=SUMIFS('YGN Receipt'!H:H,'YGN Receipt'!A:A,A" +
        Number(lastCoRow) +
        ",'YGN Receipt'!T:T,\"Valid\")+SUMIFS('MDY Receipt'!H:H,'MDY Receipt'!A:A,A" +
        Number(lastCoRow) +
        ",'MDY Receipt'!T:T,\"Valid\")+SUMIFS('PMN Receipt'!H:H,'PMN Receipt'!A:A,A" +
        Number(lastCoRow) +
        ",'PMN Receipt'!T:T,\"Valid\")",
      ...
      "=SUMIF('YGN Receipt'!A:E,A" +
        ...
        Number(lastCoRow) +
        ",'MDY Receipt'!F:F)+SUMIF('PMN Receipt'!A:E,A" +
        Number(lastCoRow) +
        ",'PMN Receipt'!F:F)",
...
let dailyExpensesCategories = () => {
  ...
  let creditAccountNum = 10002;
  let creditAccount = "Cash in Hand-Main";
