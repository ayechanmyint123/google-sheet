...
const mdytoolsrevenue = SpreadsheetApp.openById(
...
************************************/
let onEditTriggersMDYToolsShop = (e) => {
...
let saveCart = (signature, cart, e) => {
  ...
  for (const data of cartArrayTwo) {
    ...
    let cosignmentArray = invCosignments
      ...
      .getValues();
    let thirtyTwoStock = items.getRange(itemRow, 7).getValue();
    let eightyFourStock = items.getRange(itemRow, 8).getValue();
    let initialBalance;
    let currentBalance;

    if (location == "32ND") {
      initialBalance = thirtyTwoStock;
      currentBalance = thirtyTwoStock - qty;
    } else {
...
let returnItem = (invoice, itemAmount, e) => {
  ...
  flagCredit = leftamount == payamount ? false : true;
  ...
  let itemColumn = location == "32ND" ? 7 : 8;
  ...
  if (location == "32ND") {
    initialBalance = thirtyTwoStock;
    currentBalance = thirtyTwoStock + itemAmount;
  } else {
    initialBalance = eightyFourStock;
...
let dailyExpenses = (signature) => {
  ...
  for (let i = 0; i < expenseDate.length; i++) {
    ...
    if (date == checkDate && checkExpense == "MDY-TOOLS") {
...
let dailyRevenue = (signature, date, revenue, revenueLastRow) => {
  ...
  let revenueArray = [
    [date, "MDY-TOOLS", revenue, dayRevenue, dayIncome, signature, system],
  ];
  ...
  let array = [
    ...
    [, , , 96005, "Open Trade Debtor-MDY Power Tools", dayRevenue, ,],
    [, , , 70027, "Revenue-Open Power Tools (MDY)", , dayRevenue],
    [transactionNum + 2, date, systemRevenue, "Daily Income", , , ,],
    [, , , 10003, "Cash in Hand-MDY Tools", dayIncome, ,],
    [, , , 96005, "Open Trade Debtor-MDY Power Tools", , dayIncome],
  ];
  ...
  transactionsSheet
    ...
    .setValues(array);
  mdytoolsrevenue
    .getRange(mdytoolsrevenue.getLastRow() + 1, 1, 6, 6)
...
let createTransfer = (signature) => {
  ...
  newInvoiceNum = "mdytoolstransfer-" + newInvoiceNum;
...
let confirmTransfer = (invoiceNum, e) => {
  ...
  for (data of invoiceArray) {
    ...
    if (from == "32ND" || from == "84TH") {
      ...
      if (locationOne == "32ND") {
        initialBalanceOne = thirtyTwoStock;
        currentBalanceOne = thirtyTwoStock - qty;
      } else {
    ...
    if (to == "32ND" || to == "84TH") {
      ...
      if (locationTwo == "32ND") {
        initialBalanceTwo = thirtyTwoStock;
        currentBalanceTwo = thirtyTwoStock + qty;
      } else {
...
let createLocalPurchase = (signature) => {
  ...
  let newInvoiceNum = Number(invoiceNum.split("-")[1]) + 1;
  newInvoiceNum = "mdytoolsLocalPurchase-" + newInvoiceNum;
...
let dailyExpensesCategories = () => {
  ...
  let creditAccountNum = 10003;
  let creditAccount = "Cash in Hand-MDY Tools";
