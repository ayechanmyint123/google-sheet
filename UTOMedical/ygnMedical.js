...
const mdytransfers = SpreadsheetApp.openById(
  "17XWrwq2aYHatXzbi7BAhROHBj2o013WVjmdArMI14ac"
...
const movementData = SpreadsheetApp.openById(
  "199njpCStwK4VN6ViibijH_kLygXxOa0-QpsAnWUBjAY"
).getSheetByName("Movement Data");
//const creditMovement = SpreadsheetApp.openById("1D6O4QvSX0s1z5mk-Q2PMQ9r_SKQPA6t_G0maQP9-gtE").getSheetByName("Credit Payment For MDY");
...
// let test = () => {
//   console.log(ygntransfers.getSheetName());
// }
...
let saveCart = (signature, cart, e) => {
  ...
  let itemColumn = cart.getRange(10, 9).getValue() - 4;
  ...
  for (const data of cartArrayTwo) {
    ...
    let kyimyindine = items.getRange(itemRow, 9).getValue();
...
let dailyRevenue = (signature, date, revenue, revenueLastRow) => {
  ...
  let array = [
    ...
    [, , , 96007, "Open Trade Debtor-YGN Medical", dayRevenue, ,],
    [, , , 70026, "Revenue-Open Medical (YGN)", , dayRevenue],
    [transactionNum + 2, date, systemRevenue, "Daily Income", , , ,],
    [, , , 10000, "Cash in Hand-YGN Medical", dayIncome, ,],
    [, , , 96007, "Open Trade Debtor-YGN Medical", , dayIncome],
  ];
  ...
  transactionsSheet
    ...
    .setValues(array);
  ygnmedicalrevenue
    .getRange(ygnmedicalrevenue.getLastRow() + 1, 1, 6, 6)
...
let createTransfer = (signature) => {
  ...
  newInvoiceNum = "ygnmedicaltransfer-" + newInvoiceNum;
...
let createLocalPurchase = (signature) => {
  ...
  if (cashCredit == "Cash") {
    expenseSheet
      .getRange(expenseSheet.getLastRow() + 1, 1, 1, 8)
      .setValues(expenseArray);
  }
...
let dailyExpensesCategories = () => {
  ...
  let creditAccountNum = 10000;
  let creditAccount = "Cash in Hand-YGN Medical";
