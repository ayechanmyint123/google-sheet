...
const invCosignments = SpreadsheetApp.openById(
  "1kxukeI2PW5aUjK06lXZmrGrkd6a0pLpjp58-FM2rp2o"
).getSheetByName("Inv Cosignments");
const cosignments = SpreadsheetApp.openById(
  "1kxukeI2PW5aUjK06lXZmrGrkd6a0pLpjp58-FM2rp2o"
).getSheetByName("Cosignments");
const mdytransfers = SpreadsheetApp.openById(
  "1GTfzzu92rmE2y13KkyFdrzf3V249PHHJg8xkS42QXOk"
).getSheetByName("Transfers");
const mdyItems = SpreadsheetApp.openById(
  "1GTfzzu92rmE2y13KkyFdrzf3V249PHHJg8xkS42QXOk"
).getSheetByName("Items");
const movementData = SpreadsheetApp.openById(
  "1HZEhvy-kY0r2E3uw8Cdynk4-zedmbUHSmPpXDp-m4i8"
).getSheetByName("Movement Data");
const creditMovement = SpreadsheetApp.openById(
  "1GAh8yM4hHLb_xNRxFXd4CniSGCXhqaanS7_uq3iD680"
).getSheetByName("Credit Payment For YGN");
const transactionsSheet = SpreadsheetApp.openById(
  "1Zy8t1B4hO-kxFVQA_yabrUbdeZgUjTPZr8Q5MMOjW5U"
).getSheetByName("Transactions");
const gwygnmedicalrevenue = SpreadsheetApp.openById(
...
************************************/
let onEditTriggersYGNMedicalShop = (e) => {
...
let saveCart = (signature, cart, e) => {
  ...
  let invoiceNumber = cart.getRange(11, 11).getValue();
  let invoiceDate = cart.getRange(5, 11).getValue();
  let customer = cart.getRange(10, 3).getValue();
  let location = cart.getRange(7, 11).getValue();
  let companyName = cart.getRange(9, 11).getValue();
  ...
  //let discount = cart.getRange(36, 10).getValue();
  let cartArray = [];
  let cartArrayTwo = cart.getRange(14, 2, 21, 11).getValues();
  let itemColumn = cart.getRange(7, 12).getValue() - 4;
  ...
  for (const data of cartArrayTwo) {
    ...
    console.log(data[3], invCosignments.getRange(data[10], 20).getValue());
  ...
  for (const data of cartArrayTwo) {
    ...
    let cosignmentArray = invCosignments
      ...
      .getValues();
    let tgn = items.getRange(itemRow, 7).getValue();
    let dagon = items.getRange(itemRow, 8).getValue();
    let jsix = items.getRange(itemRow, 9).getValue();
    let kyimyindine = items.getRange(itemRow, 10).getValue();
    let initialBalance;
    let currentBalance;
    console.log(cartArrayTwo);
    if (location == "TGN") {
      initialBalance = tgn;
      currentBalance = tgn - qty;
    }
    if (location == "DAGON") {
      initialBalance = dagon;
      currentBalance = dagon - qty;
    }
    if (location == "JSIX") {
      initialBalance = jsix;
      currentBalance = jsix - qty;
    }
    if (location == "KyiMyinDine") {
      initialBalance = kyimyindine;
      currentBalance = kyimyindine - qty;
    }
    addItemsArray[l] = [];
    ...
    for (cosignment of cosignmentArray) {
      ...
      for (containers of separateArray) {
        ...
        cartArray[j] = [
          ...
          wholesale,
          discount,
          Math.round(containers[1] * (finalTotalPrice / qty)),
          customer,
          companyName,
          invoiceType == "Cash-0"
            ? Math.round(containers[1] * (finalTotalPrice / qty))
  ...
  invoiceCheck.getRange(4, 10).setValue(invoiceNumber);
...
let returnItem = (invoice, itemAmount, e) => {
  let date = returnCancel.getRange(8, 11).getValue();
  let location = returnCancel.getRange(6, 11).getValue();
  let customer = returnCancel.getRange(12, 5).getValue();
  let invoiceNumber = returnCancel.getRange(12, 11).getValue();
  let nextInvoiceRow = invoice.getLastRow() + 1;
  ...
  let discount = returnCancel.getRange(e.range.getRow(), 7).getValues();
  let wholesale = returnCancel.getRange(e.range.getRow(), 9).getValue();
  finalTotalPrice = (finalTotalPrice * 1 - discount) / qty;
  let company = returnCancel.getRange(4, 11).getValue();
  ...
  let dateReturn = returnCancel.getRange(10, 11).getValue();
  let tgn = items.getRange(itemRow, 7).getValue();
  let dagon = items.getRange(itemRow, 8).getValue();
  let jsix = items.getRange(itemRow, 9).getValue();
  let kyimyindine = items.getRange(itemRow, 10).getValue();
  ...
  if (location == "TGN") {
    itemColumn = 7;
    initialBalance = tgn;
    currentBalance = tgn + itemAmount;
  }
  if (location == "DAGON") {
    itemColumn = 8;
    initialBalance = dagon;
    currentBalance = dagon + itemAmount;
  }
  if (location == "JSIX") {
    itemColumn = 9;
    initialBalance = jsix;
    currentBalance = jsix + itemAmount;
  }
  if (location == "KyiMyinDine") {
    itemColumn = 10;
    initialBalance = kyimyindine;
    currentBalance = kyimyindine + itemAmount;
  }
  if (qty < 0) {
    e.range.setValue(false);
...
let cancelInvoice = (e) => {
  ...
  let invoiceNum = returnCancel.getRange(12, 11).getValue();
  let receiptArray = invoice.getRange(startInvoiceRow, 2, 21, 19).getValues();
  ...
  let stockMovementArray = [];
  if (alreadyCancel) {
    ui.alert("This invoice is already canceled.");
    return false;
  }
...
let dailyExpenses = (signature) => {
  let date = cashBook.getRange(3, 6).getValue();
  let currentDate = new Date();
  let expenses = cashBook.getRange(8, 9, 28, 4).getValues();
  ...
  for (let i = 0; i < expenseDate.length; i++) {
    ...
    if (date == checkDate && checkExpense == "YGN-MEDICAL") {
  ...
  for (let i = 0; i < expenses.length; i++) {
    ...
    expenseArray[j] = [
      ...expenseArray[j],
      cashBook.getRange(3, 6).getValue(),
      "YGN-MEDICAL",
      expense,
      amount,
      signature,
  ...
  cashBook.getRange(8, 9, 28, 4).setValue("");
  let revenue = cashBook.getRange(14, 17).getValue();
  dailyRevenue(
    signature,
    cashBook.getRange(3, 6).getValue(),
    revenue,
...
let dailyRevenue = (signature, date, revenue, revenueLastRow) => {
  let dayRevenue = cashBook.getRange(6, 17).getValue();
  let dayIncome = cashBook.getRange(10, 17).getValue();
  let system =
    revenueSheet.getRange(revenueSheet.getLastRow(), 7).getValue() + 1;
  console.log(system);
  let revenueArray = [
    [date, "YGN-MEDICAL", revenue, dayRevenue, dayIncome, signature, system],
  ];
  ...
  let array = [
    ...
    [, , , 96029, "Open Trade Debtor-YGN Medical ( GetWell )", dayRevenue, ,],
    [, , , 70033, "Revenue-Open GW Medical (YGN)", , dayRevenue],
    [transactionNum + 2, date, systemRevenue, "Daily Income", , , ,],
    [, , , 10017, "Cash in Hand-YGN Medical ( Get Well )", dayIncome, ,],
    [, , , 96029, "Open Trade Debtor-YGN Medical ( GetWell )", , dayIncome],
  ];
  ...
  transactionsSheet
    ...
    .setValues(array);
  gwygnmedicalrevenue
    .getRange(gwygnmedicalrevenue.getLastRow() + 1, 1, 6, 6)
...
let createTransfer = (signature) => {
  ...
  newInvoiceNum = "gwygntransfer-" + newInvoiceNum;
...
let confirmTransfer = (invoiceNum, e) => {
  ...
  let itemArrayTwo = [];
  let l = 0;
  let mdyTransferLastRow = mdytransfers.getLastRow() + 1;
  let stockMovementArray = [];
  ...
  for (data of invoiceArray) {
    ...
    let remark = data[8];
    let twentyTwoStock = mdyItems.getRange(itemRow, 7).getValue();
    let eightyFourStock = mdyItems.getRange(itemRow, 8).getValue();
    let tgn = items.getRange(itemRow, 7).getValue();
    let dagon = items.getRange(itemRow, 8).getValue();
    let jsix = items.getRange(itemRow, 9).getValue();
    let kyimyindine = items.getRange(itemRow, 10).getValue();
    ...
    itemArrayTwo[l] = [
      ...
      remark,
      "No",
      "=MATCH(F" + Number(mdyTransferLastRow + l) + ",Items!A:A,0)",
    ];
    ...
    if (
      ...
      if (from == "TGN") {
        from = 7;
        initialBalanceOne = tgn;
        currentBalanceOne = tgn - qty;
      }
      if (from == "DAGON") {
        from = 8;
        initialBalanceOne = dagon;
        currentBalanceOne = dagon - qty;
      }
      if (from == "JSIX") {
        from = 9;
        initialBalanceOne = jsix;
        currentBalanceOne = jsix - qty;
      }
      if (from == "KyiMyinDine") {
        from = 10;
        initialBalanceOne = kyimyindine;
        currentBalanceOne = kyimyindine - qty;
      }
      addItemsArray[l] = [];
    ...
    if (to == "TGN" || to == "DAGON" || to == "JSIX" || to == "KyiMyinDine") {
      let initialBalanceTwo;
      let currentBalanceTwo;

      if (to == "TGN") {
        to = 7;
        initialBalanceTwo = tgn;
        currentBalanceTwo = tgn + qty;
      }
      if (to == "DAGON") {
        to = 8;
        initialBalanceTwo = dagon;
        currentBalanceTwo = dagon + qty;
      }
      if (to == "JSIX") {
        to = 9;
        initialBalanceTwo = jsix;
        currentBalanceTwo = jsix + qty;
      }
      if (to == "KyiMyinDine") {
        to = 10;
        initialBalanceTwo = kyimyindine;
        currentBalanceTwo = kyimyindine + qty;
      }
      addItemsArrayTwo[l] = [];
  ...
  if (to == "22ND" || to == "84TH") {
    mdytransfers
      .getRange(mdyTransferLastRow, 1, itemArrayTwo.length, 12)
      .setValues(itemArrayTwo);
    mdytransfers
      .getRange(mdyTransferLastRow, 1, itemArrayTwo.length, 1)
      .insertCheckboxes();
  }
...
let createLocalPurchase = (signature) => {
  ...
  let newInvoiceNum = Number(invoiceNum.split("-")[1]) + 1;
  newInvoiceNum = "YgnMedLocalPurchase-" + newInvoiceNum;
  ...
  for (data of invoiceArray) {
    ...
    cosignmentArray[j] = [
      ...
      date,
      qty,
      "=SUMIFS('GWYgn Medical Receipt'!H:H,'GWYgn Medical Receipt'!A:A,A" +
        Number(lastCoRow + j) +
        ",'GWYgn Medical Receipt'!T:T,\"Valid\")+SUMIFS('GWMdy Medical Receipt'!H:H,'GWMdy Medical Receipt'!A:A,A" +
        Number(lastCoRow + j) +
        ",'GWMdy Medical Receipt'!T:T,\"Valid\")",
...
let dailyExpensesCategories = () => {
  ...
  let creditAccountNum = 10017;
  let creditAccount = "Cash in Hand-YGN Medical ( Get Well )";
