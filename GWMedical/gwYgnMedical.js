...
const ygntransfers = SpreadsheetApp.openById(
  "1prkWlHhIPOsS7cYPiI_lVEUIxIZAJwi_x72ZroMitxs"
).getSheetByName("Transfers");
const ygnItems = SpreadsheetApp.openById(
  "1prkWlHhIPOsS7cYPiI_lVEUIxIZAJwi_x72ZroMitxs"
).getSheetByName("Items");
// const movementData = SpreadsheetApp.openById("1HZEhvy-kY0r2E3uw8Cdynk4-zedmbUHSmPpXDp-m4i8").getSheetByName("Movement Data");
// const creditMovement = SpreadsheetApp.openById("1GAh8yM4hHLb_xNRxFXd4CniSGCXhqaanS7_uq3iD680").getSheetByName("Credit Payment For MDY");
const transactionsSheet = SpreadsheetApp.openById(
  "1Zy8t1B4hO-kxFVQA_yabrUbdeZgUjTPZr8Q5MMOjW5U"
).getSheetByName("Transactions");
// const gwmdymedicalrevenue = SpreadsheetApp.openById("19p7RwxSVMcaVePlRgUq3jLdI3E_gpJq9erSohFc1fKk").getSheetByName("All Revenue");
...
let onEditCart = (e) => {
  ...
  if (saveCart(signature, cart) == false) {
...
let saveCart = (signature, cart) => {
  ...
  let itemColumn = cart.getRange(7, 12).getValue() - 3;
...
let returnItem = (invoice, itemAmount, e) => {
  ...
  let discount = returnCancel.getRange(e.range.getRow(), 7).getValue();
  ...
  //movementData.getRange(movementData.getLastRow()+1,1,stockMovementArray.length,9).setValues(stockMovementArray);
  e.range.setValue(false);
};
...
let cancelInvoice = (e) => {
  ...
  let date = returnCancel.getRange(10, 11).getValue();
...
let dailyExpenses = (signature) => {
  ...
  let revenue = cashBook.getRange(15, 17).getValue();
  dailyRevenue(
    signature,
    cashBook.getRange(3, 6).getValue(),
    revenue,
...
let dailyRevenue = (signature, date, revenue, revenueLastRow) => {
  ...
  let dayIncome = cashBook.getRange(11, 17).getValue();
  let system =
  ...
  let array = [
    ...
    [, , , 96030, "Open Trade Debtor-MDY Medical ( GetWell )", dayRevenue, ,],
    [, , , 70034, "Revenue-Open GW Medical (MDY)", , dayRevenue],
    [transactionNum + 2, date, systemRevenue, "Daily Income", , , ,],
    [, , , 10018, "Cash in Hand-MDY Medical ( Get Well )", dayIncome, ,],
    [, , , 96030, "Open Trade Debtor-MDY Medical ( GetWell )", , dayIncome],
  ];
  ...
  transactionsSheet
    ...
    .setValues(array);
  gwmdymedicalrevenue
    .getRange(gwmdymedicalrevenue.getLastRow() + 1, 1, 6, 6)
...
let createTransfer = (signature) => {
  ...
  newInvoiceNum = "gwmdytransfer-" + newInvoiceNum;
...
let confirmTransfer = (invoiceNum, e) => {
  ...
  for (data of invoiceArray) {
    ...
    let jsix = ygnItems.getRange(itemRow, 9).getValue();
...
let dailyExpensesCategories = () => {
  ...
  let creditAccountNum = 10018;
  let creditAccount = "Cash in Hand-MDY Medical ( Get Well )";
