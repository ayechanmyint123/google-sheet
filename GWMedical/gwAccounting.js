...
************************************/
let onEditGWMedicalAccountingTriggers = (e) => {
  return e.source.getActiveSheet().getName() == "Cosignment Input" &&
    e.range.getRow() == 10 &&
    e.range.getColumn() == 3
    ? onEditGWNewCosMedicalAccounting(e)
...
let onEditGWNewCosMedicalAccounting = (e) => {
...
let newCos = (signature) => {
  ...
  for (var i = 0; i < 20; i++) {
    ...
    if (locationToDrop == "KyiMyinDine") {
      sheet = ygnGWItem;
      itemCol = 10;
    }
    if (locationToDrop == "22ND") {
      sheet = mdyGWItem;
      itemCol = 7;
    }
    ...
    cosArray[j] = [
      ...
      date,
      qty,
      "=SUMIFS('GWYgn Medical Receipt'!D:D,'GWYgn Medical Receipt'!A:A,A" +
        Number(lastCoRow + j) +
        ",'GWYgn Medical Receipt'!F:F,\"Valid\")+SUMIFS('GWMdy Medical Receipt'!D:D,'GWMdy Medical Receipt'!A:A,A" +
        Number(lastCoRow + j) +
        ",'GWMdy Medical Receipt'!F:F,\"Valid\")",
      ...
      locationToDrop,
      rate,
      "=E" + Number(lastCoRow + j) + "*H" + Number(lastCoRow + j) + "",
      ...
      "=H" +
        ...
        Number(lastCoRow + j) +
        ',"+")))',
      "=H" + Number(lastCoRow + j) + "*G" + Number(lastCoRow + j) + "",
    ];

    let initialBalance = ygnGWItem.getRange(itemRow, itemCol).getValue();
...
let sendCreditEmail = (department) => {
  ...
  if (department == "ygn") {
    ...
    GmailApp.sendEmail(
      "accounting@utotools.com,utomedicalmdy@gmail.com",
      "Credit Vouchers Left",
      "Please open this email with a client that supports HTML",
      { htmlBody: htmlForEmail }
    );
...
let submitCosting = (e) => {
  ...
  let allAccounts = accounts
    ...
    .getValues();
  let medicalContainerCostingAccountRow;
  for (let i = 0; i < allAccounts.length; i++) {
    if (allAccounts[i][0] == "92000") {
      medicalContainerCostingAccountRow = i;
    }
  }
  let initialValue = accounts
    .getRange(medicalContainerCostingAccountRow + 1, 7)
    .getValue();
...
let addCosting = (e) => {
  ...
  console.log(cosignmentNewArray);
  for (data of cosignmentArray) {
    if (data[0] == invoice) {
      cosignmentRowCount++;
    }
